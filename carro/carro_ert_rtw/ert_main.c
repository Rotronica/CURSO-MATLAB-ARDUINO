...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) carro_M->Timing.clockTick0;
  carro_step();
...
int main(void)
{
  ...
  errorCode = extmodeInit(carro_M->extModeInfo, &rtmGetTFinal(carro_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(carro_M, true);
    }
  }
