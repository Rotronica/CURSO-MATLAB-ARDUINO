...
void rt_OneStep(void)
{
  ...
  currentTime = (extmodeSimulationTime_T) adc_M->Timing.clockTick0;
  adc_step();
...
int main(void)
{
  ...
  errorCode = extmodeInit(adc_M->extModeInfo, &rtmGetTFinal(adc_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(adc_M, true);
    }
  }
